...
Updates: 
- CLEAN METADATA RELIANCE: Reads exact config directly from HDF5 file.
- VISUALS: Aspect ratio restored. Light legends inside, Charge legends flattened at the bottom.
"""
...
def process_acquisition_task(payload):
    ...
    try:
        ...
        for ax, title, channels in layout:
            ...
            is_charge = 'Charge' in title
            if is_charge:
                # Flat bottom legend for dense charge channels
                ax.legend(fontsize='x-small', ncol=8, loc='upper center', bbox_to_anchor=(0.5, -0.15), frameon=False)
            else:
                # Standard internal legend for light channels
                if len(channels) > 10: 
        ...
        # Protect bottom margin for legends
