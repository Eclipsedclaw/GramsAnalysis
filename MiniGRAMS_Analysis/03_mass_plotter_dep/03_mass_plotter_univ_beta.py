...
LArTPC Mass PDF Compiler (Universal Edition - Absolute Indexing Safe)
- Supports Single-Board (MicroGRAMS) and Multi-Board (MiniGRAMS) HDF5s.
- Safely ignores NaN-padded channels from Absolute CAEN Indexing.
...
def get_label(ch_idx, run_config):
    ...
    elif run_config == 'TUCSON_WV':
        bridge = {
            19: "CAEN 19 (VUV_1)", 35: "CAEN 35 (VUV_2)",
            2:  "CAEN 2 (VIS_1)",   4: "CAEN 4 (VIS_2)"
        }
        return bridge.get(ch_idx, f"CAEN {ch_idx}")
...
def plot_event_to_pdf(f, event_idx, mapping, filename, pdf, is_single_board, run_config):
    """Generates the 4-panel plot. Safely skips pure NaN channels."""
    ...
    if is_single_board:
        ...
        max_ch = f['Board_72']['waveforms'].shape[1]

    master_max_t = (master_samples * master_dt) / 1000.0
    
    # Filter out out-of-bounds channels
    vuv_chans = [c for c in vuv_chans if c < max_ch]
    vis_chans = [c for c in vis_chans if c < max_ch]
    x_chans = [c for c in x_chans if c < max_ch]
    y_chans = [c for c in y_chans if c < max_ch]
    ...
    def plot_group(ax, chans, title, is_sipm=False):
        ...
        for ch in chans:
            ...
            if np.all(np.isnan(v)): continue # Skip unrecorded padding channels
    ...
    axes[0,0].set_ylim(valid_c_min - 5, valid_c_max + 5)
    axes[0,1].set_ylim(valid_c_min - 5, valid_c_max + 5)
